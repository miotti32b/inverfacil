    ...
    el.innerHTML =
      ...
      filas
        ...
        <li class="tf-fila-lb">
          ...
          <span class="tf-fila-lb__nombre">${f.nombre}</span>
    ...
    async function refrescar() {
      ...
      if (data.estado_partida === "lobby") {
        ...
        els.lobbyLista.innerHTML = (data.lobby_jugadores || [])
          .map(
            (j) =>
              `<li class="tf-fila-lb"><span class="tf-badge tf-badge--${j.tipo}">${j.tipo === "equipo" ? "Equipo" : "Individual"}</span><span class="tf-fila-lb__nombre">${j.nombre}</span><span class="tf-tenue">${j.tipo === "equipo" ? j.miembros + " integrante(s)" : ""}</span></li>`
          )
          .join("");
